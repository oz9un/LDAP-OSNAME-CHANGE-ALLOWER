...
class AceRights extends Flags
{
    ...
    // OZGUN EDITED:
    public function readGroup($action = null)
    {
        return 
            $this->readProperty($action) && 
            $this->readSecurity($action) && 
            $this->listChildObject($action);
    }

    /**
     * Check or set the ability to perform write group operations on the object like Windows AD properties menu.
     *
     * @param null|bool $action
     * @return $this|bool
     */
    public function writeGroup($action = null)
    {
        return 
            $this->writeProperty($action) && 
            $this->validatedWrite($action);
    }
    // OZGUN EDIT FINISHED
    /**
